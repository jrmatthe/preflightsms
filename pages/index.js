...
function withAiTimeout(promise, ms = 30000) {
  return Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error("AI request timed out — please try again")), ms)),
  ]);
}
...
function NavBar({ currentView, setCurrentView, orgLogo, orgName, userName, onSignOut, org, userRole, notifications, notifReads, onMarkNotifRead, onMarkAllNotifsRead, profile, isOnline, session, onNotifNavigate, onUpgrade, onSwitchToMobile, onUpdatePreferences, showOnboarding, onboardingState, onStartFlow, isTrial, onStartFresh, activeFlow, showTour, tourState, tourFlows, tourOrder, onStartTour, onDismissTour, activeTour }) {
  ...
  const visibleSections = NAV_SECTIONS.filter(sec => {
    ...
    if (sec.id === "compliance") return isAdminRole || userPerms.includes("view_compliance");
    if (sec.id === "analytics") return isAdminRole || userPerms.includes("view_analytics");
    if (sec.id === "investigations") return isAdminRole || userPerms.includes("view_investigations");
  ...
  return (<>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => {
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                ...
                <span style={{ fontSize: 10, transition: "transform 0.2s", display: "inline-block", transform: menuOpen === "onboarding" && !onboardingClosing ? "rotate(180deg)" : "rotate(0)" }}>v</span>
              </div>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => {
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                ...
                <span style={{ fontSize: 10, transition: "transform 0.2s", display: "inline-block", transform: menuOpen === "tour" && !onboardingClosing ? "rotate(180deg)" : "rotate(0)" }}>v</span>
              </div>
              <div style={{ display: "flex", gap: 3, width: "100%" }}>
                {Array.from({ length: totalSteps }).map((_, i) => (
                  <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i < doneSteps ? "#22D3EE" : BORDER }} />
    ...
    <header className="nav-mobile-header" style={{ display: "none", background: BLACK, borderBottom: `1px solid ${BORDER}`, position: "sticky", top: 0, zIndex: 100, padding: "0 16px", alignItems: "center", justifyContent: "space-between" }}>
      <img src="/pf360-logo.png" alt="Preflight 360" style={{ width: 140, objectFit: "contain" }} />
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <button className="nav-hamburger" onClick={() => setMenuOpen(!menuOpen)}
          ...
          {menuOpen === true ? "Close" : "Menu"}</button>
      </div>
    </header>
    {menuOpen === true && (<div className="nav-mobile-menu" style={{ display: "none", flexDirection: "column", padding: "8px 16px", gap: 2, background: NEAR_BLACK, borderBottom: `1px solid ${BORDER}`, position: "sticky", top: 48, zIndex: 99 }}>
      {visibleSections.map(sec => {
        ...
        <button key={sec.id} onClick={() => {
          ...
          {sec.label}
          {isGated && <span style={{ fontSize: 9, opacity: 0.3, marginLeft: "auto", letterSpacing: 0.5 }}>PRO</span>}
        </button>);
      })}
      {userName && (<>
          ...
          <button onClick={async () => {
            if (!aiPanelOpen) {
              ...
              if (aiSuggestions.length === 0 && (fi.departure || fi.destination || fi.aircraft)) {
                ...
                try {
                  const { data, error } = await withAiTimeout(supabase.functions.invoke('ai-frat-suggestions', {
                    body: { orgId, departure: fi.departure, destination: fi.destination, aircraft: fi.aircraft, flightDate: fi.date, etd: fi.etd }
                  }));
                  if (!error && !data?.error && data?.suggestions) setAiSuggestions(data.suggestions);
    ...
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px 32px" }}>
      ...
      {myScheduledFlights.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          {sectionHeader("Scheduled", myScheduledFlights.length)}
      ...
      {myActive.length > 0 && (
        <div data-tour="tour-my-active" style={{ marginBottom: 28 }}>
          {sectionHeader("Active", myActive.length)}
          <div style={{ display: "grid", gap: 10 }}>
            {myActive.map(f => {
      ...
      {myRecent.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          {sectionHeader("Recent", myRecent.length)}
          <div style={{ display: "grid", gap: 10 }}>
            {myRecent.map(f => {
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto" }}>
      <div style={{ ...card, padding: 26 }}>
        ...
        {[{ title: "Summary Report", desc: "Overview with risk distribution and top hazard factors.", action: genSummary, btn: "Download .txt" },
          { title: "Standard Export", desc: "All FRAT records with scores and metadata for Excel.", action: genCSV, btn: "Download CSV" },
          { title: "Detailed Factor Export", desc: "Every record with individual risk factor columns.", action: genDetailed, btn: "Download CSV" }].map((r, i) => (
      ...
      <div style={{ padding: "10px 14px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        {dragHandleProps && (
          <div {...dragHandleProps} style={{ cursor: "grab", opacity: 0.3, transition: "opacity 0.15s", display: "flex", alignItems: "center", flexShrink: 0 }} onMouseEnter={e => e.currentTarget.style.opacity = "1"} onMouseLeave={e => e.currentTarget.style.opacity = "0.3"}>
            <span style={{ fontSize: 11, color: MUTED, letterSpacing: 1 }}>:::</span>
          </div>
        )}
...
function HomeView({ profile, profiles, frats, flights, reports, actions, hazards, auditSchedules, auditTemplates, trainingRequirements, trainingRecords, policies, mocItems, erpPlans, erpDrills, onNavigate, org, session, myTodayFlights, onSelectFfFlight, onSelectScTrip, cultureSurveys, mySurveyResponseIds, asapCorrActions }) {
  ...
  const openInvestigations = (hazards || []).filter(Boolean).filter(h => h.status === "identified" || h.status === "assessed" || h.status === "unacceptable");
  ...
  const trainingStatusIcon = (s) => s === "overdue" ? "!" : s === "expiring" ? "..." : s === "current" ? "OK" : "--";
  const trainingCard = (
...
function DashboardWrapper({ records, flights, reports, hazards, actions, onDelete, riskLevels, org, erpPlans, erpDrills, profile, session, spis, spiMeasurements, onCreateSpi, onUpdateSpi, onDeleteSpi, onLoadTargets, onCreateTarget, onUpdateTarget, onDeleteTarget, onLoadMeasurements, onCreateMeasurement, onInitSpiDefaults, cultureSurveys, orgProfiles, onCreateSurvey, onUpdateSurvey, onDeleteSurvey, onFetchSurveyResponses, onSubmitSurveyResponse, onCheckUserSurveyResponse, onFetchSurveyResults, onUpsertSurveyResults, trendAlerts, onAcknowledgeTrendAlert, complianceFrameworks, complianceChecklistItems, complianceStatusData, trainingReqs, trainingRecs, policies, iepAudits, auditSchedules, mocItems, insuranceExports, onGenerateExport, onDeleteExport, onNavigateSubscription, onNavigate, fleetAircraft, part5Compliance, onViewDetail }) {
  const userPerms = profile?.permissions || [];
  const analyticsOn = ["admin", "safety_manager", "accountable_exec", "chief_pilot"].includes(profile?.role) || userPerms.includes("view_analytics");
  ...
  const isAdmin = ["admin", "safety_manager", "accountable_exec", "chief_pilot"].includes(profile?.role) || userPerms.includes("view_analytics");
...
function LandingPage() {
  ...
  const FEATURES = [
    { title: "Flight Risk Assessment", desc: "Configurable FRAT with weighted scoring, risk thresholds, and approval workflows." },
    { title: "Flight Following", desc: "Real-time flight tracking with status updates, ETA monitoring, and arrival confirmation." },
    { title: "Safety Reporting", desc: "Confidential hazard, incident, and near-miss reporting with status tracking." },
    { title: "Investigation Register", desc: "Structured safety investigation with severity/likelihood risk matrix scoring." },
    { title: "Corrective Actions", desc: "Track risk controls from identification through completion with due dates." },
    { title: "FAA Part 5 Audit Log", desc: "42-point compliance checklist mapped to every Part 5 requirement with evidence tracking." },
    { title: "Policy & Training", desc: "Document library with acknowledgment tracking and training record management." },
  ];
...
function SignupFlow({ onAuth }) {
  ...
  // Direct checkout mode: ?signup&subscribe skips trial, goes straight to payment
  const directCheckout = useMemo(() => {
    if (typeof window === "undefined") return false;
    return new URLSearchParams(window.location.search).has("subscribe");
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      // 4. Direct checkout: redirect to Stripe instead of signing in
      if (directCheckout && selectedPlan !== "free") {
        // Sign in first to get a session for the Stripe call
        ...
        if (signInData?.session) {
          try {
            const { data: checkoutData, error: checkoutErr } = await supabase.functions.invoke("stripe-checkout", {
              body: { plan: selectedPlan, interval: billingInterval, orgId: orgJson.data.id, orgName: orgName.trim(), email, returnUrl: window.location.origin },
            });
            if (checkoutErr) throw new Error(checkoutErr.message);
            if (checkoutData?.url) { window.location.href = checkoutData.url; return; }
          } catch (e) {
            // Checkout failed — fall through to normal sign-in so they're not stuck
            console.error("Stripe checkout error:", e);
            onAuth(signInData.session);
          }
        } else {
          setError("Account created! Check your email to confirm, then log in to subscribe.");
        }
      } else {
        // Normal flow: sign in to app
  ...
  const slides = {
    1: { label: "Let\u2019s Get You Set Up", headline: "You\u2019re About to Have a Real SMS.", body: "No more spreadsheets. No more binders. In a few minutes you\u2019ll have a fully operational safety management system \u2014 the kind that impresses your POI and makes life easier for your pilots.", proofs: [
      { title: "Part 5 Compliant From Day One", desc: "Every feature maps to a 14 CFR Part 5 requirement. You\u2019re already ahead." },
      { title: "Live Today, Not Next Quarter", desc: "Your fleet, your FRATs, your safety reports \u2014 all running before lunch." },
      { title: "No Credit Card. No Strings.", desc: "Full Professional access for 14 days. Just see what it can do." },
    ]},
    2: { label: "Almost There", headline: "Your Pilots Are Going to Love This.", body: "90 seconds from the ramp. No app to download. FRATs that actually get filled out \u2014 because we made it dead simple. Your crew will thank you.", proofs: [
      { title: "Works on Any Device", desc: "Phone, tablet, crew room computer \u2014 open a browser and go." },
      { title: "Everything Has an Owner", desc: "Every hazard report becomes an action item. Nothing falls through the cracks." },
      { title: "Anonymous Reporting Built In", desc: "Real just culture. Pilots report without fear. You see what\u2019s actually happening." },
    ]},
    3: { label: "One More Step", headline: "This Is Going to Change How You Run Safety.", body: "Operators who switch to PreflightSMS stop dreading audits and start running a safety program they\u2019re proud of. You\u2019re about to be one of them.", proofs: [
      { title: "Everything Unlocked", desc: "FRAT, flight following, crew management, CBT, hazard tracking, analytics \u2014 all of it." },
      { title: "Real SMS. Not Enterprise Prices.", desc: "Starts at $149/mo. Not the $1,500 the airline platforms charge." },
    ]},
  };
        ...
        <div className="signup-right-panel" style={{ display: "flex", flexDirection: "column", justifyContent: "center", padding: "48px 44px" }}>
          <div style={{ maxWidth: 440, margin: "0 auto", width: "100%" }}>
            ...
            {step === 1 && (<>
              ...
              <p style={{ fontSize: 13, color: MUTED, margin: "0 0 28px" }}>{directCheckout ? "Set up your account to get started." : "Full Professional access for 14 days. No credit card."}</p>
              <div style={{ marginBottom: 14 }}>
            ...
            {step === 3 && (<>
              {directCheckout ? (<>
                ...
                <p style={{ fontSize: 13, color: MUTED, margin: "0 0 24px" }}>You&apos;ll be redirected to our secure checkout to complete payment.</p>
                {/* Plan selector */}
                <div className="signup-plan-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
                  {[{ id: "starter", name: "Starter", price: 149, annualPrice: 1490, desc: "Up to 5 aircraft", features: ["FRAT & Flight Following", "Safety Reporting", "Policy Library", "Dashboard"] },
                    { id: "professional", name: "Professional", price: 349, annualPrice: 3490, desc: "Up to 15 aircraft", features: ["Everything in Starter", "Custom FRAT Templates", "CBT Modules", "FAA Audit Log", "API & Webhooks"] }
                  ].map(p => {
                    const isSelected = selectedPlan === p.id;
                    const displayPrice = billingInterval === "annual" ? Math.round(p.annualPrice / 12) : p.price;
                    return (
                      <button key={p.id} onClick={() => setSelectedPlan(p.id)} style={{ ...card, padding: "18px 20px", cursor: "pointer", textAlign: "left", border: isSelected ? `2px solid ${GREEN}` : `1px solid ${BORDER}`, background: isSelected ? "rgba(74,222,128,0.04)" : "rgba(255,255,255,0.02)", borderRadius: 12, position: "relative", width: "100%" }}>
                        {isSelected && <div style={{ position: "absolute", top: -8, right: 10, fontSize: 8, fontWeight: 700, color: BLACK, background: GREEN, padding: "2px 8px", borderRadius: 3 }}>SELECTED</div>}
                        <div style={{ fontSize: 15, fontWeight: 700, color: WHITE, marginBottom: 2 }}>{p.name}</div>
                        <div style={{ fontSize: 10, color: MUTED, marginBottom: 10 }}>{p.desc}</div>
                        <div style={{ marginBottom: 10 }}><span style={{ fontSize: 22, fontWeight: 800, color: WHITE, fontFamily: "Georgia,serif" }}>${displayPrice}</span><span style={{ fontSize: 11, color: MUTED }}>/mo</span></div>
                        {p.features.map((f, i) => <div key={i} style={{ fontSize: 10, color: OFF_WHITE, padding: "2px 0" }}>{f}</div>)}
                      </button>
                    );
                  })}
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: 6, marginBottom: 16 }}>
                  {[["monthly", "Monthly"], ["annual", "Annual (save 17%)"]].map(([id, label]) => (
                    <button key={id} onClick={() => setBillingInterval(id)} style={{ padding: "8px 16px", borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: "pointer", background: billingInterval === id ? "rgba(255,255,255,0.08)" : "transparent", color: billingInterval === id ? WHITE : MUTED, border: `1px solid ${billingInterval === id ? "rgba(255,255,255,0.15)" : BORDER}` }}>{label}</button>
                  ))}
                </div>
                <div style={{ fontSize: 11, color: MUTED, textAlign: "center" }}>Have a promo code? You can enter it at checkout.</div>
              </>) : (<>
                  ...
                  <div style={{ ...card, padding: "20px 22px", marginBottom: 16, border: `2px solid ${GREEN}`, background: "rgba(255,255,255,0.03)", position: "relative" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 16px" }}>
                      {["FRAT & Flight Following", "Safety Reporting", "Investigation Register", "Policy Library", "Dashboard Analytics", "FAA Audit Log", "Custom FRAT Templates", "CBT Modules", "Approval Workflows", "API & Webhooks"].map((f, i) => (
                        <div key={i} style={{ fontSize: 10, color: OFF_WHITE, padding: "2px 0", display: "flex", alignItems: "center", gap: 5 }}>{f}</div>
                      ))}
                    </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
              {step > 1 && <button onClick={() => { setStep(step - 1); setError(""); }} style={{ padding: "13px 20px", background: "transparent", color: MUTED, border: `1px solid ${BORDER}`, borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Back</button>}
              <button onClick={next} disabled={loading} style={{ flex: 1, padding: "13px 0", background: "rgba(201,169,110,0.08)", color: GOLD, border: "1px solid rgba(201,169,110,0.3)", borderRadius: 8, fontWeight: 700, fontSize: 14, cursor: loading ? "wait" : "pointer", opacity: loading ? 0.7 : 1, letterSpacing: 0.3 }}>
                {loading ? (directCheckout && step === 3 ? "Redirecting to checkout..." : "Setting up...") : step === 3 ? (directCheckout ? `Subscribe to ${selectedPlan === "professional" ? "Professional" : "Starter"}` : selectedPlan === "free" ? "Start Free Plan" : "Start Professional Trial") : "Continue"}</button>
            </div>
...
export default function PVTAIRFrat() {
  ...
  const [hazardsTotalCount, setHazardsTotalCount] = useState(0);
  const [hazardsPage, setHazardsPage] = useState(0);
  ...
  const [completionsTotalCount, setCompletionsTotalCount] = useState(0);
  const [completionsPage, setCompletionsPage] = useState(0);
  // Legacy aliases used by components that haven't migrated
  const trainingReqs = trainings;
  const trainingRecs = completions;
  ...
  useEffect(() => {
    ...
    const conditions = {
      ...
      training: cbtCourses.length > 0 || trainings.length > 0,
  ...
  const handleStartFlow = useCallback(async (flowId) => {
    ...
    if (flowId === "investigations") {
      ...
      demoHazardsRef.current = [
        { id: "DEMO-HAZ-001", hazard_code: "HAZ-DEMO-001", title: "Runway incursion during taxi at KSFF", description: "Ground vehicle crossed active runway while aircraft was cleared for takeoff.", initial_likelihood: 4, initial_severity: 5, status: "unacceptable", category: "ground_ops", created_at: thirtyDaysAgo, org_id: profile?.org_id },
        { id: "DEMO-HAZ-002", hazard_code: "HAZ-DEMO-002", title: "Fuel contamination found during preflight", description: "Water detected in fuel sample from left tank during routine preflight check.", initial_likelihood: 3, initial_severity: 5, status: "assessed", category: "maintenance", created_at: thirtyDaysAgo, org_id: profile?.org_id },
        { id: "DEMO-HAZ-003", hazard_code: "HAZ-DEMO-003", title: "Near mid-air collision in practice area", description: "Two aircraft came within 200ft vertically in uncontrolled practice area. Neither had ADS-B out.", initial_likelihood: 4, initial_severity: 4, residual_likelihood: 2, residual_severity: 4, mitigations: "Updated SOPs and conducted refresher training for all pilots.", status: "mitigated", category: "airspace", created_at: sixtyDaysAgo, org_id: profile?.org_id },
        { id: "DEMO-HAZ-004", hazard_code: "HAZ-DEMO-004", title: "Bird strike on approach to KBOI", description: "Single bird struck nose cowling at 1000ft AGL on visual approach. Minor paint damage only.", initial_likelihood: 2, initial_severity: 4, status: "acceptable", category: "wildlife", created_at: thirtyDaysAgo, org_id: profile?.org_id },
        { id: "DEMO-HAZ-005", hazard_code: "HAZ-DEMO-005", title: "Checklist deviation during engine start", description: "Pilot skipped magneto check step; caught by right-seat observer before taxi.", initial_likelihood: 2, initial_severity: 2, status: "closed", category: "human_factors", created_at: sixtyDaysAgo, org_id: profile?.org_id },
      ];
  ...
  const [asapReportsTotalCount, setAsapReportsTotalCount] = useState(0);
  const [asapReportsPage, setAsapReportsPage] = useState(0);
  ...
  // Part 5 compliance for dashboard (include manual overrides so it matches Compliance page)
  ...
  useEffect(() => {
    const onStorage = (e) => { if (e.key === "audit_overrides") { try { setAuditOverrides(JSON.parse(e.newValue) || {}); } catch {} } };
    window.addEventListener("storage", onStorage);
    return () => window.removeEventListener("storage", onStorage);
  }, []);
  const part5Compliance = useMemo(() =>
    computePart5Compliance({ frats: records, flights, reports, hazards, actions, policies, profiles: orgProfiles, trainingRecords: completions, smsManuals, manualOverrides: auditOverrides }),
    [records, flights, reports, hazards, actions, policies, orgProfiles, completions, smsManuals, auditOverrides]
  );
  ...
  const refreshAllData = (orgId) => {
    ...
    fetchReports(orgId).then(({ data, count }) => { setReports(data || []); setReportsTotalCount(count || 0); setReportsPage(0); });
    fetchHazards(orgId).then(({ data, count }) => { setHazards(data || []); setHazardsTotalCount(count || 0); setHazardsPage(0); });
    fetchActions(orgId).then(({ data, count }) => { setActions(data || []); setActionsTotalCount(count || 0); setActionsPage(0); });
    fetchPolicies(orgId).then(({ data }) => setPolicies(data || []));
    fetchTrainings(orgId).then(({ data }) => setTrainings(data || []));
    fetchCompletions(orgId).then(({ data, count }) => { setCompletions(data || []); setCompletionsTotalCount(count || 0); setCompletionsPage(0); });
  ...
  const loadMoreReports = useCallback(async () => {
    ...
    const { data, count } = await fetchReports(profile.org_id, { page: nextPage });
    if (data?.length) { setReports(prev => [...prev, ...data]); setReportsPage(nextPage); setReportsTotalCount(count || 0); }
  }, [profile, reportsPage]);

  const loadMoreHazards = useCallback(async () => {
    if (!profile?.org_id) return;
    const nextPage = hazardsPage + 1;
    const { data, count } = await fetchHazards(profile.org_id, { page: nextPage });
    if (data?.length) { setHazards(prev => [...prev, ...data]); setHazardsPage(nextPage); setHazardsTotalCount(count || 0); }
  }, [profile, hazardsPage]);

  const loadMoreActions = useCallback(async () => {
    if (!profile?.org_id) return;
    const nextPage = actionsPage + 1;
    const { data, count } = await fetchActions(profile.org_id, { page: nextPage });
    if (data?.length) { setActions(prev => [...prev, ...data]); setActionsPage(nextPage); setActionsTotalCount(count || 0); }
  }, [profile, actionsPage]);

  const loadMoreCompletions = useCallback(async () => {
    if (!profile?.org_id) return;
    const nextPage = completionsPage + 1;
    const { data, count } = await fetchCompletions(profile.org_id, { page: nextPage });
    if (data?.length) { setCompletions(prev => [...prev, ...data]); setCompletionsPage(nextPage); setCompletionsTotalCount(count || 0); }
  }, [profile, completionsPage]);

  const loadMoreAsapReports = useCallback(async () => {
    if (!profile?.org_id) return;
    const nextPage = asapReportsPage + 1;
    const { data, count } = await fetchAsapReports(profile.org_id, { page: nextPage });
    if (data?.length) { setAsapReports(prev => [...prev, ...data]); setAsapReportsPage(nextPage); setAsapReportsTotalCount(count || 0); }
  }, [profile, asapReportsPage]);

  // ── Server-side search ──
  const searchReports = useCallback(async (query) => {
    if (!profile?.org_id) return;
    if (!query) {
      const { data, count } = await fetchReports(profile.org_id);
      setReports(data || []); setReportsTotalCount(count || 0); setReportsPage(0);
      return;
    }
    const { data, count } = await fetchReports(profile.org_id, { search: query, pageSize: 200 });
    setReports(data || []); setReportsTotalCount(count || 0); setReportsPage(0);
  }, [profile]);

  const searchHazards = useCallback(async (query) => {
    if (!profile?.org_id) return;
    if (!query) {
      const { data, count } = await fetchHazards(profile.org_id);
      setHazards(data || []); setHazardsTotalCount(count || 0); setHazardsPage(0);
      return;
    }
    const { data, count } = await fetchHazards(profile.org_id, { search: query, pageSize: 200 });
    setHazards(data || []); setHazardsTotalCount(count || 0); setHazardsPage(0);
  }, [profile]);

  const searchActions = useCallback(async (query) => {
    if (!profile?.org_id) return;
    if (!query) {
      const { data, count } = await fetchActions(profile.org_id);
      setActions(data || []); setActionsTotalCount(count || 0); setActionsPage(0);
      return;
    }
    const { data, count } = await fetchActions(profile.org_id, { search: query, pageSize: 200 });
    setActions(data || []); setActionsTotalCount(count || 0); setActionsPage(0);
  }, [profile]);

  const searchAsapReports = useCallback(async (query) => {
    if (!profile?.org_id) return;
    if (!query) {
      const { data, count } = await fetchAsapReports(profile.org_id);
      setAsapReports(data || []); setAsapReportsTotalCount(count || 0); setAsapReportsPage(0);
      return;
    }
    const { data, count } = await fetchAsapReports(profile.org_id, { search: query, pageSize: 200 });
    setAsapReports(data || []); setAsapReportsTotalCount(count || 0); setAsapReportsPage(0);
  }, [profile]);
  ...
  useEffect(() => {
    ...
    if (hasFeature(profile?.organizations, "asap_program")) {
      ...
      fetchAsapReports(orgId).then(({ data, count }) => { setAsapReports(data || []); setAsapReportsTotalCount(count || 0); setAsapReportsPage(0); });
      ...
      if (arrivedFlight?.dbId) {
        ...
        if (!hasTerminal && activeFlow !== "flights") {
          ...
          if (hasFeature(profile?.organizations, "ai_features") && arrivedFlight.dbId) {
            withAiTimeout(supabase.functions.invoke("ai-nudge-suggestion", {
  ...
  // Inline corrective action creation — stays on the investigation card
  const onCreateActionInline = useCallback(async (action) => {
    ...
    setActions(data || []);
    if (action.reportId) {
      await updateReport(action.reportId, { status: "corrective_action", investigation_notes: `Corrective action: ${action.title}` });
      const { data: rpts } = await fetchReports(profile.org_id);
      setReports(rpts || []);
    }
    createNotification(profile.org_id, { type: "action_created", title: "New Corrective Action", body: `Corrective action created: ${action.title || actionCode}`, link_tab: "actions", target_roles: ["admin", "safety_manager"] });
    if (action.assignedTo && action.assignedTo !== session.user.id) {
      createNotification(profile.org_id, { type: "action_assigned", title: "Corrective Action Assigned", body: `You have been assigned ${actionCode}: ${action.title}`, link_tab: "actions", target_user_id: action.assignedTo });
    }
    setToast({ message: `${actionCode} created`, level: { bg: "rgba(74,222,128,0.15)", border: "rgba(74,222,128,0.4)", color: "#4ADE80" } }); setTimeout(() => setToast(null), 3000);
  }, [profile, isOnline, actions]);

  // Inline action status update — stays on the investigation card
  const onUpdateActionInlineStatus = useCallback(async (actionId, newStatus) => {
    if (!isOnline || !profile) return;
    // If completing/cancelling, handle linked report auto-close
  ...
  const onUpdatePolicyHandler = useCallback(async (policy) => {
    if (isOnline && profile) {
      const policyId = policy._id;
      let fileUrl = undefined, fileName = undefined;
      if (policy.file) {
        const { url, error: uploadErr } = await uploadPolicyFile(profile.org_id, policy.file);
        if (uploadErr) { setToast({ message: `Upload error: ${uploadErr.message}`, level: DEFAULT_RISK_LEVELS.CRITICAL }); setTimeout(() => setToast(null), 4000); return; }
        fileUrl = url;
      ...
      const updates = {
        ...
        version: policy.version, content: policy.content, status: policy.status,
        effective_date: policy.effectiveDate || null, review_date: policy.reviewDate || null,
        part5_tags: policy.part5Tags,
      };
      if (fileUrl) { updates.file_url = fileUrl; updates.file_name = fileName; }
      const { error } = await updatePolicy(policyId, updates);
      if (error) { setToast({ message: `Error: ${error.message}`, level: DEFAULT_RISK_LEVELS.CRITICAL }); setTimeout(() => setToast(null), 4000); return; }
      if (policy._versionChanged) { await clearPolicyAcknowledgments(policyId); }
      const { data } = await fetchPolicies(profile.org_id);
      setPolicies(data || []);
      setToast({ message: policy._versionChanged ? "Document updated — acknowledgments reset" : "Document updated", level: { bg: "rgba(74,222,128,0.15)", border: "rgba(74,222,128,0.4)", color: "#4ADE80" } }); setTimeout(() => setToast(null), 3000);
    }
  }, [profile, session, isOnline]);
  ...
  // ── Training ──
  const onCreateTraining = useCallback(async (training) => {
    if (isOnline && profile) {
      const { error } = await createTraining(profile.org_id, training);
      if (error) { setToast({ message: `Error: ${error.message}`, level: DEFAULT_RISK_LEVELS.CRITICAL }); setTimeout(() => setToast(null), 4000); return; }
      const { data } = await fetchTrainings(profile.org_id);
      setTrainings(data || []);
    }
  }, [profile, isOnline]);

  const onUpdateTraining = useCallback(async (id, updates) => {
    if (isOnline && profile) {
  ...
  const onDeleteTraining = useCallback(async (id) => {
    if (isOnline && profile) {
      await deleteTraining(id);
      const { data } = await fetchTrainings(profile.org_id);
      setTrainings(data || []);
      const { data: c } = await fetchCompletions(profile.org_id);
      setCompletions(c || []);
    }
  }, [profile, isOnline]);

  const onLogCompletion = useCallback(async (completion) => {
    if (isOnline && profile) {
      const { error } = await createCompletion(profile.org_id, {
        courseId: completion.courseId || completion.trainingId || completion.requirementId,
        userId: completion.userId || session.user.id,
        completedDate: completion.completedDate,
        instructor: completion.instructor,
        notes: completion.notes,
      });
      if (error) { setToast({ message: `Error: ${error.message}`, level: DEFAULT_RISK_LEVELS.CRITICAL }); setTimeout(() => setToast(null), 4000); return; }
      const { data } = await fetchCompletions(profile.org_id);
      setCompletions(data || []);
      setToast({ message: "Completion logged", level: { bg: "rgba(74,222,128,0.15)", border: "rgba(74,222,128,0.4)", color: "#4ADE80" } }); setTimeout(() => setToast(null), 3000);
    }
  }, [profile, session, isOnline]);

  const onDeleteCompletion = useCallback(async (id) => {
    if (isOnline && profile) {
  ...
  // Legacy handler aliases for components that still use old prop names
  const onCreateRequirement = onCreateTraining;
  const onLogTraining = onLogCompletion;
  const onDeleteTrainingRecord = onDeleteCompletion;
  const onDeleteRequirement = onDeleteTraining;
  ...
  const onInitTraining = useCallback(async (courseDefs) => {
    if (!isOnline || !profile) return;
    const orgId = profile.org_id;
    const userId = session.user.id;
    for (const tmpl of courseDefs) {
