...
const SetupChecklist = dynamic(() => import("../components/SetupChecklist"), { ssr: false });
...
function DashboardWrapper({ records, flights, reports, hazards, actions, onDelete, riskLevels, org, erpPlans, erpDrills, profile, session, spis, spiMeasurements, onCreateSpi, onUpdateSpi, onDeleteSpi, onLoadTargets, onCreateTarget, onUpdateTarget, onDeleteTarget, onLoadMeasurements, onCreateMeasurement, onInitSpiDefaults, cultureSurveys, orgProfiles, onCreateSurvey, onUpdateSurvey, onDeleteSurvey, onFetchSurveyResponses, onSubmitSurveyResponse, onCheckUserSurveyResponse, onFetchSurveyResults, onUpsertSurveyResults, trendAlerts, onAcknowledgeTrendAlert, pilotEngagement, safetyRecognitions, orgEngagement, orgRecognitions, onAcknowledgeRecognition, complianceFrameworks, complianceChecklistItems, complianceStatusData, trainingReqs, trainingRecs, policies, iepAudits, auditSchedules, mocItems, insuranceExports, onGenerateExport, onDeleteExport, onNavigateSubscription, onNavigate, fleetAircraft, part5Compliance, onViewDetail, setupChecklistState, onSetupChecklistSave, onSetupChecklistDismiss, onSetupChecklistNavigate }) {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto" }}>
      ...
      {/* Setup Checklist */}
      {setupChecklistState && !setupChecklistState.completed_at && !setupChecklistState.dismissed_at && isAdmin && (
...
export default function PVTAIRFrat() {
  ...
  const [setupChecklistState, setSetupChecklistState] = useState(null);
  ...
  // Setup checklist — initialize after onboarding is complete
  useEffect(() => {
    if (!profile || !isAdmin || !isOnline) return;
    const settings = profile.organizations?.settings || {};
    if (!settings.onboarding_completed) return;
    // Only show to orgs created after feature ship date
    const createdAt = profile.organizations?.created_at;
    if (!createdAt || new Date(createdAt) < new Date("2026-03-04T00:00:00Z")) return;
    const existing = settings.setup_checklist;
    if (existing) {
      if (!existing.dismissed_at && !existing.completed_at) setSetupChecklistState(existing);
      else setSetupChecklistState(null);
      return;
    }
    // Initialize fresh checklist
    const fresh = { started_at: new Date().toISOString(), completed_at: null, dismissed_at: null, minimized: false, items: {}, xp_earned: 0 };
    setSetupChecklistState(fresh);
    saveOnboardingStatus(profile.org_id, { setup_checklist: fresh });
  }, [profile, isAdmin, isOnline]);

  const handleSetupChecklistSave = (newState) => {
    setSetupChecklistState(newState);
    if (profile?.org_id) saveOnboardingStatus(profile.org_id, { setup_checklist: newState });
    setProfile(prev => prev ? { ...prev, organizations: { ...prev.organizations, settings: { ...(prev.organizations?.settings || {}), setup_checklist: newState } } } : prev);
  };
  const handleSetupChecklistDismiss = () => {
    const dismissed = { ...setupChecklistState, dismissed_at: new Date().toISOString() };
    setSetupChecklistState(null);
