...
function NavBar({ currentView, setCurrentView, isAuthed, orgLogo, orgName, userName, onSignOut, org, userRole, notifications, notifReads, onMarkNotifRead, onMarkAllNotifsRead, profile, isOnline, session, onNotifNavigate }) {
  ...
  const tabs = [
    ...
    if (t.id === "audits" && !isAdmin) return false;
    if (t.id === "moc" && !isAdmin) return false;
  ...
  return (<>
    ...
    <header className="nav-mobile-header" style={{ display: "none", background: BLACK, borderBottom: `1px solid ${BORDER}`, position: "sticky", top: 0, zIndex: 100, padding: "0 16px", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        {isOnline && session && <NotificationCenter notifications={notifications} reads={notifReads} onMarkRead={onMarkNotifRead} onMarkAllRead={onMarkAllNotifsRead} profile={profile} onNavigate={(tab, linkId) => { if (onNotifNavigate) onNotifNavigate(tab, linkId); else setCurrentView(tab); setMenuOpen(false); }} />}
  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 16, flexWrap: "wrap" }}>
        {[["analytics", "Overview"], ...(hasAnalytics ? [["frat", "FRAT Analytics"], ["safety", "Safety Metrics"]] : []), ...(hasSpi ? [["performance", "Performance"]] : []), ...(hasCulture ? [["culture", "Safety Culture"]] : []), ...(hasInsurance ? [["insurance", "Insurance & Export"]] : []), ["history", "FRAT History"], ["export", "Export"]].map(([id, label]) => (
      ...
      {sub === "analytics" && <DashboardCharts records={records} flights={flights} reports={reports} hazards={hazards} actions={actions} riskLevels={riskLevels} view="overview" erpPlans={erpPlans} erpDrills={erpDrills} spis={spis} spiMeasurements={spiMeasurements} trendAlerts={trendAlerts} onAcknowledgeTrendAlert={onAcknowledgeTrendAlert} />}
...
export default function PVTAIRFrat() {
  ...
  useEffect(() => {
    ...
    for (const sched of auditSchedulesData) {
      if (!sched.is_active || !sched.next_due_date) continue;
      const due = new Date(sched.next_due_date + "T00:00:00");
      const daysUntil = Math.ceil((due - now) / (1000 * 60 * 60 * 24));
      const linkId = `audit_schedule_${sched.id}`;
      const existing = notifications.find(n => n.link_id === linkId);
      if (existing) continue;
      if (daysUntil < 0) {
        createNotification(orgId, { type: "audit_overdue", title: "Audit Overdue", body: `Scheduled audit is ${Math.abs(daysUntil)} day(s) overdue`, link_tab: "audits", link_id: linkId, target_roles: ["admin", "safety_manager"] });
      } else if (daysUntil <= 7) {
        createNotification(orgId, { type: "audit_due", title: "Audit Due Soon", body: `Scheduled audit due in ${daysUntil} day(s)`, link_tab: "audits", link_id: linkId, target_roles: ["admin", "safety_manager"] });
      }
    }
  }, [auditSchedulesData, profile, notifications]);
  ...
  useEffect(() => {
    ...
    for (const moc of mocItems) {
      if (!moc.review_date || moc.status === "closed") continue;
      const due = new Date(moc.review_date + "T00:00:00");
      const daysUntil = Math.ceil((due - now) / (1000 * 60 * 60 * 24));
      const linkId = `moc_review_${moc.id}`;
      const existing = notifications.find(n => n.link_id === linkId);
      if (existing) continue;
      if (daysUntil >= 0 && daysUntil <= 3) {
        createNotification(orgId, { type: "moc_review_due", title: "Change Review Due", body: `Review due in ${daysUntil} day(s): ${moc.title}`, link_tab: "moc", link_id: linkId, target_roles: ["admin", "safety_manager"] });
      }
    }
  }, [mocItems, profile, notifications]);
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 32px 0" }}>
          <div>
            <h1 style={{ margin: 0, color: WHITE, fontSize: 22, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase" }}>
              {cv === "submit" ? "NEW FLIGHT RISK ASSESSMENT" : cv === "flights" ? "FLIGHT FOLLOWING" : cv === "reports" ? "SAFETY REPORTS" : cv === "erp" ? "EMERGENCY RESPONSE" : cv === "audits" ? "INTERNAL EVALUATION PROGRAM" : cv === "moc" ? "MANAGEMENT OF CHANGE" : cv === "hazards" ? "INVESTIGATIONS" : cv === "actions" ? "CORRECTIVE ACTIONS" : cv === "policy" ? "POLICIES" : cv === "cbt" ? "TRAINING" : cv === "audit" ? "FAA PART 5 AUDIT" : cv === "dashboard" ? "SAFETY DASHBOARD" : cv === "admin" ? "ADMIN" : ""}
            </h1>
          </div>
          <div className="user-info-desktop" style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {isOnline && session && (<>
              <NotificationCenter notifications={notifications} reads={notifReads} onMarkRead={onMarkNotifRead} onMarkAllRead={onMarkAllNotifsRead} profile={profile} onNavigate={(tab, linkId) => { if (linkId) { if (profile?.org_id) refreshAllData(profile.org_id); setFratDetailId(linkId); } else { setCv(tab); } }} />
