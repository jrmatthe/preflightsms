...
export default function PVTAIRFrat() {
  ...
  const onLogCompletion = useCallback(async (completion) => {
    if (isOnline && profile) {
      const { error } = await createCompletion(profile.org_id, {
        trainingId: completion.trainingId || completion.requirementId,
  ...
  const onInitTraining = useCallback(async (trainingDefs, courses) => {
    if (!isOnline || !profile) return;
    const orgId = profile.org_id;
    const userId = session.user.id;
    for (const def of trainingDefs) {
      await createTraining(orgId, def);
    }
    ...
    const { data: t } = await fetchTrainings(orgId);
    setTrainings(t || []);
    await refreshCbt();
  ...
  return (
    ...
    <div style={{ minHeight: "100vh", background: DARK, fontFamily: "'Segoe UI', system-ui, -apple-system, sans-serif" }}>
      ...
      <div className="main-content" style={{ marginLeft: 140 }}>
        ...
        {cv === "cbt" && <CbtModules profile={profile} session={session} orgProfiles={orgProfiles} courses={boardCbtCourses} lessons={cbtLessonsMap} progress={cbtProgress} enrollments={cbtEnrollments} onCreateCourse={roGuard(onCreateCbtCourse)} onUpdateCourse={onUpdateCbtCourse} onDeleteCourse={async (id) => { await deleteCbtCourse(id); refreshCbt(); }} onSaveLesson={roGuard(onSaveCbtLesson)} onDeleteLesson={onDeleteCbtLesson} onUpdateProgress={onUpdateCbtProgress} onUpdateEnrollment={onUpdateCbtEnrollment} onPublishCourse={onUpdateCbtCourse} onRefresh={refreshCbt} trainings={trainings} completions={completions} onCreateTraining={roGuard(onCreateTraining)} onUpdateTraining={roGuard(onUpdateTraining)} onDeleteTraining={roGuard(onDeleteTraining)} onLogCompletion={roGuard(onLogCompletion)} onDeleteCompletion={roGuard(onDeleteCompletion)} trainingRequirements={trainingReqs} trainingRecords={trainingRecs} onCreateRequirement={roGuard(onCreateRequirement)} onLogTraining={roGuard(onLogTraining)} onDeleteTrainingRecord={roGuard(onDeleteTrainingRecord)} onDeleteRequirement={roGuard(onDeleteRequirement)} onInitTraining={roGuard(onInitTraining)} orgProfiles={orgProfiles} />}
