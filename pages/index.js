        ...
        <div style={{ ...card, padding: "12px 18px", marginBottom: 14 }}>
          {!melDeferOpen ? (
            ...
            <div>
              ...
              <input value={melDeferForm.description} onChange={e => setMelDeferForm(p => ({ ...p, description: e.target.value }))} placeholder="Description (required)" style={{ width: "100%", padding: "7px 10px", background: NEAR_BLACK, border: `1px solid ${BORDER}`, borderRadius: 5, color: WHITE, fontSize: 11, marginBottom: 8, boxSizing: "border-box" }} />
              <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                <input value={melDeferForm.mel_reference} onChange={e => setMelDeferForm(p => ({ ...p, mel_reference: e.target.value }))} placeholder="MEL Ref (optional)" style={{ flex: 1, padding: "7px 10px", background: NEAR_BLACK, border: `1px solid ${BORDER}`, borderRadius: 5, color: WHITE, fontSize: 11, boxSizing: "border-box" }} />
                <select value={melDeferForm.category} onChange={e => setMelDeferForm(p => ({ ...p, category: e.target.value }))} style={{ padding: "7px 10px", background: NEAR_BLACK, border: `1px solid ${BORDER}`, borderRadius: 5, color: WHITE, fontSize: 11 }}>
                  {Object.keys(CATEGORY_LIMITS).map(c => <option key={c} value={c}>Cat {c}</option>)}
                </select>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 10, color: MUTED }}>Expires: {melDeferExpiration || "As specified"}</span>
              </div>
              <input value={melDeferForm.notes} onChange={e => setMelDeferForm(p => ({ ...p, notes: e.target.value }))} placeholder="Notes (optional)" style={{ width: "100%", padding: "7px 10px", background: NEAR_BLACK, border: `1px solid ${BORDER}`, borderRadius: 5, color: WHITE, fontSize: 11, marginBottom: 10, boxSizing: "border-box" }} />
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => { setMelDeferOpen(false); setMelDeferForm({ description: "", mel_reference: "", category: "C", notes: "" }); }} style={{ flex: 1, padding: "7px", borderRadius: 5, fontSize: 11, fontWeight: 600, background: "transparent", border: `1px solid ${BORDER}`, color: MUTED, cursor: "pointer" }}>Cancel</button>
                <button onClick={handleDeferMel} disabled={!melDeferForm.description.trim() || melDeferSaving} style={{ flex: 1, padding: "7px", borderRadius: 5, fontSize: 11, fontWeight: 600, background: melDeferForm.description.trim() ? CYAN : `${CYAN}44`, border: "none", color: BLACK, cursor: melDeferForm.description.trim() ? "pointer" : "not-allowed" }}>{melDeferSaving ? "Saving..." : "Defer"}</button>
              </div>
            </div>
          )}
        </div>
