// /api/trial-emails — Daily trial lifecycle emails
// Sends getting_started (day 2), mid_trial (day 7), expiring_soon (day 11), trial_expired (day 14)
...
export default async function handler(req, res) {
  ...
  const fromEmail = process.env.FROM_EMAIL || "PreflightSMS <noreply@preflightsms.com>";
  ...
  try {
    ...
    if (!trialOrgs || trialOrgs.length === 0) {
      return res.status(200).json({ message: "No trial orgs found", sent: 0 });
    }
    ...
    for (const org of trialOrgs) {
      ...
      try {
        await sendEmail(resendApiKey, fromEmail, admin.email, template.subject, template.html(org.name));
    ...
    return res.status(200).json({ message: "Trial emails processed", sent: results.length, details: results });
  } catch (err) {
