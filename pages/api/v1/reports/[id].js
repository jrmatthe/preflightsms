...
export default async function handler(req, res) {
  ...
  if (!hasPermission(apiKey, "reports:read")) {
    logApiRequest(supabase, apiKey.id, org.id, "GET", `/api/v1/reports/${req.query.id}`, 403, startTime);
  ...
  logApiRequest(supabase, apiKey.id, org.id, "GET", `/api/v1/reports/${id}`, qErr ? (qErr.code === "PGRST116" ? 404 : 500) : 200, startTime);
  if (qErr) {
    if (qErr.code === "PGRST116") return res.status(404).json({ error: "Report not found" });
