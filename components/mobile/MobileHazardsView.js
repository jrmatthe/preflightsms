    ...
    <button onClick={() => setExpanded(!expanded)} style={{
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, color: scoreColor, fontWeight: 600, textTransform: "uppercase" }}>{riskLabel(displayScore)}</div>
        </div>
        ...
        <div style={{ padding: "0 14px 14px", borderTop: `1px solid ${BORDER}` }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
            <div style={{ ...cardStyle, flex: 1, padding: 10, textAlign: "center", background: BLACK }}>
              <div style={{ fontSize: 10, color: MUTED, textTransform: "uppercase", marginBottom: 2 }}>Initial Risk</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: riskColor(initialScore) }}>{initialScore || "—"}</div>
              {initialScore > 0 && <div style={{ fontSize: 10, color: MUTED }}>L{hazard.initial_likelihood} × S{hazard.initial_severity}</div>}
            </div>
            {residualScore > 0 && (
              <>
                ...
                <div style={{ ...cardStyle, flex: 1, padding: 10, textAlign: "center", background: BLACK }}>
                  <div style={{ fontSize: 10, color: MUTED, textTransform: "uppercase", marginBottom: 2 }}>Residual Risk</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: riskColor(residualScore) }}>{residualScore}</div>
                  <div style={{ fontSize: 10, color: MUTED }}>L{hazard.residual_likelihood} × S{hazard.residual_severity}</div>
                </div>
              </>
            )}
          </div>
          ...
          {hazard.mitigations && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 }}>Mitigations</div>
          ...
          {linkedActions.length > 0 && (
            <div style={{ marginTop: 14 }}>
              <div style={{ fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>Corrective Actions</div>
              {linkedActions.map(a => (
                <div key={a.id} style={{
                  ...
                  <div style={{ flex: 1, fontSize: 13, color: OFF_WHITE }}>{a.title}</div>
                  <span style={{
                    fontSize: 10, fontWeight: 600, color: a.status === "completed" ? GREEN : a.status === "overdue" ? RED : MUTED,
