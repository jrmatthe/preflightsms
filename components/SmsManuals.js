...
const TEMPLATE_VARIABLES = [
  ...
  { group: "Chief Pilot", vars: [
    { key: "Chief Pilot Name", label: "Full Name" },
    { key: "CP Title", label: "Title" },
    { key: "CP Phone", label: "Phone" },
    { key: "CP Email", label: "Email" },
  ]},
  { group: "Director of Operations", vars: [
    { key: "Director of Operations Name", label: "Full Name" },
    { key: "DO Title", label: "Title" },
    { key: "DO Phone", label: "Phone" },
    { key: "DO Email", label: "Email" },
  ]},
...
function TemplateVariablesForm({ variables, onSave, fleetAircraft }) {
  ...
  const [saveMode, setSaveMode] = useState(null); // "save" or "apply"
      ...
      {expanded && (
        <div style={{ padding: "0 16px 16px" }}>
          <div style={{ fontSize: 10, color: MUTED, lineHeight: 1.5, marginBottom: 12 }}>
            Fill in your organization's details below. Save to store your values, or Save & Apply to also replace all [bracketed placeholders] throughout your manuals.
          </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
            <button onClick={() => handleSave(false)} disabled={!!saveMode}
              style={{ padding: "10px 24px", background: "transparent", color: CYAN, border: `1px solid ${CYAN}`, borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: saveMode ? "default" : "pointer", opacity: saveMode ? 0.5 : 1 }}>
              {saveMode === "save" ? "Saving..." : "Save"}
            </button>
            <button onClick={() => handleSave(true)} disabled={!!saveMode}
              style={{ padding: "10px 24px", background: CYAN, color: BLACK, border: "none", borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: saveMode ? "default" : "pointer", opacity: saveMode ? 0.5 : 1 }}>
              {saveMode === "apply" ? "Applying..." : "Save & Apply to All Manuals"}
            </button>
          </div>
        </div>
      )}
...
const SMS_MANUAL_TEMPLATES = [
  {
    ...
    cfrReferences: ["5.3", "5.21", "5.23", "5.25"],
    sections: [
      { id: "sp_definitions", title: "Part 5 Definitions", cfr_ref: "\u00A7 5.3",
        guidance: "Document the Part 5 definitions used by your organization. These standard definitions ensure a common understanding of key safety terms across all SMS documentation and communications.",
        content: "[Company Name] adopts the following definitions as established in 14 CFR Part 5 for use throughout our Safety Management System:\n\nHazard: A condition that could foreseeably cause or contribute to an aircraft accident as defined in 49 CFR 830.2.\n\nRisk: The composite of predicted severity and likelihood of the potential effect of a hazard.\n\nRisk Control: A means to reduce or eliminate the effects of hazards.\n\nSafety Assurance: Processes within the SMS that function systematically to ensure the performance and effectiveness of safety risk controls and that the organization meets or exceeds its safety objectives through the collection, analysis, and assessment of information.\n\nSafety Management System (SMS): The formal, top-down, organization-wide approach to managing safety risk and assuring the effectiveness of safety risk controls. It includes systematic procedures, practices, and policies for the management of safety risk.\n\nSafety Objective: A measurable goal or desirable outcome related to safety.\n\nSafety Performance: Realized or actual safety accomplishment relative to the organization's safety objectives.\n\nSafety Policy: The certificate holder's documented commitment to safety, which defines its safety objectives and the accountabilities and responsibilities of its employees in regards to safety.\n\nSafety Promotion: A combination of training and communication of safety information to support the implementation and operation of an SMS in an organization.\n\nSafety Risk Management: A process within the SMS composed of describing the system, identifying the hazards, and analyzing, assessing, and controlling risk.\n\nThese definitions are incorporated into all SMS documentation, training materials, and safety communications to ensure consistent understanding and application across the organization.", completed: false },
...
export default function SmsManuals({ profile, session, smsManuals, onSaveManual, onPublishManual, onInitManuals, templateVariables, signatures, onSaveVariables, onSaveSignature, fleetAircraft, embedded, readOnly }) {
  ...
  const listContent = (<>
      ...
      {!readOnly && <div data-onboarding="sms-variables"><TemplateVariablesForm variables={templateVariables} fleetAircraft={fleetAircraft} onSave={async (vars, applyToManuals) => {
        if (!applyToManuals) {
          await onSaveVariables(vars, null);
          return;
        }
