// lib/seedDemo — Creates and seeds the demo organization with realistic data.
// Re-runnable: clears and re-seeds each call. All writes are scoped to the
// demo org by slug (`cascade-charter-demo`), so this cannot touch other orgs.
...
export async function seedDemoOrg() {
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
    return { success: false, error: "Missing SUPABASE_URL or SERVICE_ROLE_KEY", log: [], errors: [] };
  }

  const DEMO_PASSWORD = process.env.DEMO_PASSWORD;
  if (!DEMO_PASSWORD) {
    return { success: false, error: "DEMO_PASSWORD not configured", log: [], errors: [] };
  }
  ...
  try {
    ...
    return { success: false, error: err.message, log, errors };
  }
}
