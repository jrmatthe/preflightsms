...
export function initOfflineQueue(onSync) {
  ...
  try {
    ...
    // Corrupted JSON — reset rather than lose future queued items
    queue = [];
...
export async function flushQueue() {
  ...
  for (const op of queue) {
    try {
      ...
      if (op.type === "frat_submit") {
        ...
        const { parkingSpot, fuelRemaining, fuelUnit, ...rest } = op.payload;
        const extra = {};
        if (parkingSpot) extra.parkingSpot = parkingSpot;
        if (fuelRemaining) { extra.fuelRemaining = fuelRemaining; extra.fuelUnit = fuelUnit || "lbs"; }
        const { error } = await updateFlightStatus(op.payload.flightDbId, op.payload.status, extra);
  ...
  if (failed.length < originalLength && onSyncCallback) {
    onSyncCallback();
  }
}
