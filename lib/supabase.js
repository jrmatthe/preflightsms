...
export async function fetchTrainingRecords(orgId) {
  if (!supabase) return { data: [], error: null };
  // Try simple query first, then attempt join for user names
...
export async function deleteTrainingRecord(id) {
  if (!supabase) return { error: null };
  return supabase.from('training_records').delete().eq('id', id);
}

export async function deleteTrainingRequirement(id) {
  if (!supabase) return { error: null };
  return supabase.from('training_requirements').delete().eq('id', id);
}
...
export async function updateAuditSchedule(scheduleId, updates) {
  ...
  console.log("[updateAuditSchedule] id:", scheduleId, "payload:", JSON.stringify(payload));
  const { data, error } = await supabase
    .from('audit_schedules')
    .update(payload)
    .eq('id', scheduleId)
    .select()
    .single();
  if (error) console.error("[updateAuditSchedule] error:", JSON.stringify(error));
  else console.log("[updateAuditSchedule] success:", JSON.stringify(data));
  return { data, error };
}
...
// ── Pilot Engagement & Gamification ─────────────────────────────

export async function fetchPilotEngagement(userId) {
  if (!supabase) return { data: [], error: null };
  const { data, error } = await supabase
...
export async function fetchOrgEngagement(orgId) {
  if (!supabase) return { data: [], error: null };
  const { data, error } = await supabase
    .from('pilot_engagement')
    .select('*')
...
export async function upsertEngagementMetric(userId, orgId, metricType, currentValue, bestValue) {
  if (!supabase) return { data: null, error: null };
  const { data, error } = await supabase
    .from('pilot_engagement')
    .upsert({
...
export async function fetchSafetyRecognitions(userId) {
  if (!supabase) return { data: [], error: null };
  const { data, error } = await supabase
...
export async function fetchOrgRecognitions(orgId) {
  if (!supabase) return { data: [], error: null };
  const { data, error } = await supabase
    .from('safety_recognitions')
    .select('*')
...
export async function awardRecognition(orgId, userId, recognitionType, title, description) {
  if (!supabase) return { data: null, error: null };
  const { data, error } = await supabase
    .from('safety_recognitions')
    .insert({
...
export async function acknowledgeRecognition(recognitionId) {
  if (!supabase) return { data: null, error: null };
  const { data, error } = await supabase
    .from('safety_recognitions')
