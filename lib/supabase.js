...
export async function fetchFRATs(orgId, { days = 90 } = {}) {
...
export async function createFlight(orgId, fratId, entry, requiresApproval = false, userId = null) {
  ...
  if (entry.depTz) row.dep_tz = entry.depTz;
  if (entry.destTz) row.dest_tz = entry.destTz;
  let { data, error } = await supabase.from('flights').insert(row).select().single();
  // Retry without timezone columns if migration hasn't been applied yet
  if (error && (entry.depTz || entry.destTz)) {
    delete row.dep_tz;
    delete row.dest_tz;
    ({ data, error } = await supabase.from('flights').insert(row).select().single());
  }
  return { data, error };
}
...
export async function resendInvitation(invitationId) {
  if (!supabase) return { data: null, error: null };
  // Reset to a fresh 30-day PENDING window and return the invitation for re-sending.
  const { data, error } = await supabase
    .from('invitations')
    .update({ status: 'pending', accepted_at: null, expires_at: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() })
    .eq('id', invitationId)
    .select()
    .single();
  return { data, error };
