...
// ── TRAINING ──────────────────────────────────────────────────
export async function createTraining(orgId, training) {
  if (!supabase) return { error: { message: 'Supabase not configured' } };
  const { data, error } = await supabase.from('trainings').insert({
    org_id: orgId,
    title: training.title,
    description: training.description || '',
    category: training.category || 'sms',
    schedule_type: training.scheduleType || (training.frequencyMonths > 0 ? 'recurring' : 'one_time'),
    frequency_months: training.frequencyMonths || 0,
    enrolled_roles: training.enrolledRoles || [],
    enrolled_users: training.enrolledUsers || [],
  }).select().single();
  return { data, error };
}

export async function fetchTrainings(orgId) {
  if (!supabase) return { data: [], error: null };
  const { data, error } = await supabase
    .from('trainings')
    .select('*')
...
export async function updateTraining(id, updates) {
  if (!supabase) return { error: null };
  return supabase.from('trainings').update({ ...updates, updated_at: new Date().toISOString() }).eq('id', id);
}

export async function deleteTraining(id) {
  if (!supabase) return { error: null };
  return supabase.from('trainings').delete().eq('id', id);
}

export async function createCompletion(orgId, completion) {
  if (!supabase) return { error: { message: 'Supabase not configured' } };
  // Fetch the training to auto-compute expires_at
  let expiresAt = null;
  if (completion.trainingId) {
    const { data: training } = await supabase.from('trainings').select('schedule_type, frequency_months').eq('id', completion.trainingId).single();
    if (training && training.schedule_type === 'recurring' && training.frequency_months > 0) {
      const d = new Date(completion.completedDate);
      d.setMonth(d.getMonth() + training.frequency_months);
      expiresAt = d.toISOString().slice(0, 10);
    }
  }
  const { data, error } = await supabase.from('training_completions').insert({
    org_id: orgId,
    training_id: completion.trainingId,
...
export async function fetchCompletions(orgId) {
  if (!supabase) return { data: [], error: null };
  ...
  if (simpleErr || !simple?.length) return { data: simple || [], error: null };
  // Enrich with training title and user name
  const { data: enriched } = await supabase
    .from('training_completions')
    .select('*, training:trainings(id, title, category, frequency_months), user:profiles(full_name)')
    .eq('org_id', orgId)
    .order('completed_date', { ascending: false });
  return { data: enriched || simple, error: null };
}
...
export const createTrainingRequirement = createTraining;
export const fetchTrainingRequirements = fetchTrainings;
export const deleteTrainingRequirement = deleteTraining;
export async function createTrainingRecord(orgId, userId, record) {
  return createCompletion(orgId, {
    trainingId: record.requirementId || record.trainingId,
    userId,
...
export async function fetchAsapReports(orgId) {
  if (!supabase) return { data: [], error: null };
  const { data, error } = await supabase
    .from('asap_reports')
    .select('*')
