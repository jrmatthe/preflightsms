...
export async function fetchReports(orgId, { page = 0, pageSize = 500 } = {}) {
...
export async function fetchHazards(orgId, { page = 0, pageSize = 50 } = {}) {
  if (!supabase) return { data: [], count: 0, error: null };
  const from = page * pageSize;
  const to = from + pageSize - 1;
  const { data, error, count } = await supabase
...
export async function fetchActions(orgId, { page = 0, pageSize = 500 } = {}) {
  if (!supabase) return { data: [], count: 0, error: null };
  const from = page * pageSize;
  const to = from + pageSize - 1;
  const { data, error, count } = await supabase
...
export async function fetchAsapReports(orgId, { page = 0, pageSize = 500 } = {}) {
  if (!supabase) return { data: [], count: 0, error: null };
  const from = page * pageSize;
  const to = from + pageSize - 1;
  const { data, error, count } = await supabase
