...
export function analyzeWeather(wx) {
  ...
  stationSummaries.forEach(s => {
    const hazards = [];
    if (flags.wx_ts) hazards.push({ label: "Thunderstorms", color: H_RED });
    if (flags.wx_ice) hazards.push({ label: "Icing", color: H_AMBER });
    if (flags.wx_turb) hazards.push({ label: "Turbulence", color: H_AMBER });
    if (flags.wx_wind_shear) hazards.push({ label: "Wind Shear", color: H_RED });
    if (flags.wx_ceiling && s.ceiling !== "CLR") hazards.push({ label: "Low Ceiling", color: H_YELLOW });
    if (flags.wx_vis && s.visibility && !s.visibility.includes("10+")) hazards.push({ label: "Low Visibility", color: H_YELLOW });
    if (flags.wx_xwind) hazards.push({ label: "Crosswind", color: H_YELLOW });
    if (flags.wx_mountain) hazards.push({ label: "High DA", color: H_AMBER });
    s.hazards = hazards;
  });
