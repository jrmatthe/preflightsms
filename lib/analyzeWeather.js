...
export function analyzeWeather(wx) {
  ...
  const so = wx.stationOrder || [];
  const ids = so.length > 0 ? so : (wx.metars || []).map(m => m.icaoId).filter(Boolean);
  const depId = (ids[0] || "").toUpperCase() || null;
  const destId = (ids[1] || ids[0] || "").toUpperCase() || null;
  ...
  // Sort by caller's station order (departure first, then destination)
  if (wx.stationOrder && wx.stationOrder.length > 0) {
    const orderMap = {};
    wx.stationOrder.forEach((id, i) => { orderMap[id.toUpperCase()] = i; });
    // Add placeholder entries for requested stations with no METAR/TAF data
    const existingStations = new Set(stationSummaries.map(s => (s.station || "").toUpperCase()));
    wx.stationOrder.forEach(id => {
      const uid = id.toUpperCase();
      if (uid && !existingStations.has(uid)) {
        stationSummaries.push({
          station: uid, type: "N/A",
          summary: "No METAR/TAF data available",
          ceiling: "—", visibility: "—", wind: "—",
          flight_rules: "N/A", hazards: [], noData: true,
        });
      }
    });
    const stationIdx = (s) => {
      const idx = orderMap[(s.station || "").toUpperCase()];
      return idx != null ? idx : 999;
    };
    stationSummaries.sort((a, b) => stationIdx(a) - stationIdx(b));
    briefItems.sort((a, b) => stationIdx(a) - stationIdx(b));
  }
