...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { orgId, departure, destination, aircraft, flightDate, etd } = body;
    if (!orgId) {
      console.error("ai-frat-suggestions: no orgId provided");
    ...
    if (!orgCheck) {
      console.error("ai-frat-suggestions: invalid orgId", orgId);
    ...
    if ((recentCalls || 0) >= 30) {
      return new Response(
        JSON.stringify({ suggestions: [], error: "Rate limit exceeded. Try again later." }),
...
[{"factor_id": "...", "factor_label": "...", "category": "...", "explanation": "...", "confidence": 0.0-1.0}]`;
    ...
    if (!claudeRes.ok) {
      ...
      return new Response(
        JSON.stringify({ suggestions: [], error: `Claude API returned ${claudeRes.status}` }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
