...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { orgId, title, description, category, source, mitigations } = body;
    if (!orgId) {
      console.error("ai-risk-assessment: no orgId provided");
    ...
    if (!orgCheck) {
      console.error("ai-risk-assessment: invalid orgId", orgId);
