...
| **MEL / Inoperative-Equipment Tracking** | Logs equipment deferrals by category A–D with auto-computed expiration dates and a full rectification audit trail, and synchronizes deferrals directly from **Veryon** — PVTAIR's maintenance-tracking system of record — so pilots always see current, authoritative MEL status without manual re-entry. | § 91.213 / § 135.179 Inoperative/inoperable instruments and equipment (MEL) |
...
| **Veryon Maintenance Integration** | Synchronizes MEL and deferred-maintenance items from **Veryon** — the maintenance-tracking platform PVTAIR uses — into the fleet and FRAT, keeping airworthiness deferrals current straight from the maintenance system of record (no double entry). | Supports § 91.213 / § 135.179 (MEL) · § 135.413 (responsibility for airworthiness) |
...
4. **MEL authorization and maintenance data are external.** MEL deferral tracking presumes PVTAIR holds the appropriate MEL authorization (e.g., LOA D095). **Veryon** is the maintenance system of record for deferrals — PreflightSMS synchronizes that data and applies it operationally for pilots, but does not replace the approved maintenance-tracking program or grant the authorization.
